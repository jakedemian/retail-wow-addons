...
do
	XPRACTICE.FATESCRIBEMULTIPLAYER.wmnumbers={{1,"BLUESQUARE"},{2,"GREENTRIANGLE"},{3,"PURPLEDIAMOND"},{4,"REDX"},{5,"YELLOWSTAR"},{6,"ORANGECIRCLE"},{7,"SILVERMOON"},{8,"WHITESKULL"}}
	XPRACTICE.FATESCRIBEMULTIPLAYER.wmmodels={REDX=456041,PURPLEDIAMOND=456039,GREENTRIANGLE=456037,YELLOWSTAR=456043,SILVERMOON=1014628,BLUESQUARE=456035,ORANGECIRCLE=1014619,WHITESKULL=1014641}
	
	local super=XPRACTICE.GameObject
	XPRACTICE.FATESCRIBEMULTIPLAYER.WorldMarkerController=XPRACTICE.inheritsFrom(super)
	local class=XPRACTICE.FATESCRIBEMULTIPLAYER.WorldMarkerController
	...
	function class:LoadAll(scenario,send)
		if(scenario.multiplayer)then
			for index=1,8 do		--TODO: some code overlap with Handle_WORLDMARKER
				local wmname=XPRACTICE.FATESCRIBEMULTIPLAYER.wmnumbers[index][2]
				...
				if(x and y)then
					...
					obj.displayobject.drawable:SetModelByFileID(XPRACTICE.FATESCRIBEMULTIPLAYER.wmmodels[wmname])	-- raid_ui_fx_
	...
	function SlashCmdList.XPRACTICE(msg,editbox)
		...
		if(#XPRACTICE.games>0)then
			...
			if(game.scenario and game.scenario.worldmarkercontroller and game.scenario.player)then 		
				...
				print("This scenario does not support custom world markers.")
			end
		end
